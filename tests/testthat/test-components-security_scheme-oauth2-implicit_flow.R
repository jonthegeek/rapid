test_that("class_oauth2_implicit_flow() requires compatible lengths", {
  expect_snapshot(
...
test_that("class_oauth2_implicit_flow() returns an empty oauth2_implicit_flow", {
  expect_snapshot(class_oauth2_implicit_flow())
})

test_that("class_oauth2_implicit_flow() requires names for optionals", {
  expect_snapshot(
...
test_that("class_oauth2_implicit_flow() errors informatively for bad classes", {
  expect_snapshot(
...
test_that("class_oauth2_implicit_flow() returns expected objects", {
  expect_snapshot({
    test_result <- class_oauth2_implicit_flow(
...
test_that("length() of an oauth2_implicit_flow reports the overall length", {
  expect_equal(length(class_oauth2_implicit_flow()), 0)
  expect_equal(length(class_oauth2_implicit_flow("A")), 1)
  expect_equal(
    length(class_oauth2_implicit_flow("A", scopes = c("a" = "a", "b" = "b"))),
    1
  )
})

test_that("as_oauth2_implicit_flow() errors for unnamed input", {
  expect_snapshot(
...
test_that("as_oauth2_implicit_flow() works for oauth2_implicit_flow", {
  expect_identical(
    as_oauth2_implicit_flow(class_oauth2_implicit_flow()),
    class_oauth2_implicit_flow()
  )
})
